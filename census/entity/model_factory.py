...
def evaluate_classification_model(model_list: list, X_train:np.ndarray, y_train:np.ndarray, X_test:np.ndarray, y_test:np.ndarray, base_accuracy:float=0.6) -> MetricInfoArtifact:
    """
    Description:
    This function compare multiple classfication problems return best model
    ...
    try:
        ...
        for model in model_list:
            ...
            logging.info(f"Train accuracy\t\t Test Accuracy\t\t Average accuracy")
            ...
            logging.info(f"Train precision score: [{train_precision}].")
